...
| 💬 **텔레그램 양방향 명령어** | `/잔고`, `/목록`, `/분석 {코드}` 입력 시 봇이 즉시 답장 (보안: CHAT_ID 검증) |
| 🌙 **잔고 스냅샷** | 야간/휴장 시 마지막 장 마감 잔고를 `balance_snapshot.json`에서 복원 |
| 📊 **일일 AI 브리핑** | 매일 16:00 Claude AI가 보유 종목 매수/홀딩/매도 분석 → 텔레그램 발송 |
| 📅 **일별 수익 추적** | 당일 시가 대비 현재 평가금액 변화를 4번째 메트릭으로 표시 |
| 📈 **백테스트 뷰어** | 과거 OHLCV 기반 알림 규칙 검증 — 발동일 +1/+3/+5일 수익률 적중률 표 |
...
| **H** | SMA5 상향 돌파 SMA20 (골든크로스) **AND** 거래량 증가 | ✨ 골든크로스 — 상승 추세 전환 |
| **SL** | 현재가 ≤ 설정 손절 목표가 (`stop_loss_price`) | 🔴 손절경고 — 손절가 이탈 |
...
streamlit run app.py
       ...
       └─ Telegram Listener Thread (telegram_cmd.py, daemon=True)
             ...
             ├─ /잔고 → 보유 종목 수익률 요약 답장
             ├─ /목록 → 감시 종목 리스트 답장
             └─ /분석 {종목코드|종목명} → 기술적 지표(RSI/SMA20/BB하단/거래량) 답장
```
...
💰 내 계좌 요약
  총 매수금액 | 총 평가금액 | 총 수익률 | 오늘 변화(당일 P&L)
  [보유 종목 상세 테이블]
...
──── 사이드바 ────────────────────────────────────
  관심 종목 추가: 코드, 종목명, 익절 목표가, 손절 목표가
```

**백테스트 뷰어** (별도 페이지: `pages/1_백테스트.py`)
```
streamlit run app.py  # 앱 실행 후 좌측 네비게이션 "📈 백테스트" 클릭

[종목 선택]  [조회 기간 슬라이더]  [검증 규칙 멀티셀렉트]
[▶ 백테스트 실행]

가격 차트 (SMA5/SMA20)

[규칙 탭별 결과]
  +1일 적중률 | +3일 적중률 | +5일 적중률
  [발동일 / 발동가 / +1/+3/+5일(%) 컬러 테이블]
...
### 4-3. 손절 목표가 설정 (Rule SL)

사이드바 **"관심 종목 추가"** 폼에서 **손절 목표가** 필드에 입력하거나,
`watchlist.json`에서 직접 설정합니다:
...
```json
{
  "314130": {
    "name": "지놈앤컴퍼니",
    "target_price": null,
    "stop_loss_price": 8000
  }
}
```

- 현재가가 `stop_loss_price` 이하로 떨어지면 🔴 손절경고 알림 발송
- `stop_loss_price`를 `null` 또는 `0`으로 설정하면 비활성화
- 가격이 회복되면 플래그가 자동 초기화되어 다시 감지 가능

### 4-4. 트레일링 스탑 설정 (Rule F)
...
- `/목록` — 감시 중인 종목 리스트 답장
- `/분석 005930` — 해당 종목의 RSI/SMA20/BB하단/거래량 기술적 지표 답장 (종목코드 또는 종목명 모두 가능)
...
**Q. 오늘 변화(당일 P&L) 메트릭은 어떻게 계산되나요?**
A. 당일 첫 잔고 조회 시점의 총 평가금액을 `open_amt`로 기록합니다.
이후 조회마다 `(현재 평가금액 - open_amt) / open_amt × 100`으로 수익률을 계산합니다.
날짜가 바뀌면 자동으로 초기화되며, `status.json`에 저장되어 봇 재시작 시 복구됩니다.

**Q. 백테스트 뷰어는 어떻게 사용하나요?**
A. Streamlit 앱 실행 후 좌측 네비게이션에서 **"📈 백테스트"** 를 클릭합니다.
종목, 조회 기간, 검증할 규칙을 선택하고 [▶ 백테스트 실행] 버튼을 누르면
과거 발동 날짜와 이후 +1/+3/+5 거래일 수익률을 색상 표로 확인할 수 있습니다.
