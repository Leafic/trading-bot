# ==============================================================
#  pages/2_스캘핑.py — 볼린저밴드 낙폭과대 스캘핑 백테스트
#
#  전략 핵심 (3가지 조건 동시 충족 - 빠르고 강한 반등 노림):
#    1. 가격: 볼린저 밴드 하단선(Lower Band) 터치 또는 이탈 (낙폭 과대)
#    2. RSI: 35 이하 (과매도 구간 진입)
#    3. Stochastic(스토캐스틱): %K가 20 이하 (초단기 과매도)
#
#  청산 조건 (먼저 충족되는 것):
#    - 단기 슈팅: 볼린저 밴드 상단 터치 또는 RSI 70 이상 도달 시
#    - 시간 청산: N일 경과
#    - 손절: 진입가 대비 -N%
# ==============================================================
...
def detect_signals(
    ...
    for i in range(1, len(df)):
        ...
        # 조건 1: 종가가 BB 하단선 근처이거나 이탈했을 때 (하단선 대비 +2% 이내 접근 시)
...
# 매매 상세 기록
st.dataframe(trades.style.format({"진입가": "{:,.0f}", "청산가": "{:,.0f}", "수익률(%)": "{:+.2f}"}), use_container_width=True)
...
def _scalping_bot_loop(
    ...
    """백그라운드 스케줄러 루프.
    매일 15:10에 진입 체크, 15:20에 시장가 주문 실행.
    보유 중이면 매일 15:10에 청산 조건도 함께 체크합니다."""
    from api_handler import (
        get_mock_broker, refresh_mock_broker,
        get_ohlcv_dataframe, get_current_price,
        place_market_order, get_deposit,
    )
    from strategy import (
        calculate_indicators, is_bull_market_filter,
        check_scalping_signal_daily, check_exit_condition,
    )
    ...
    while not state["stop_event"].is_set():
        ...
        for symbol, info in watch_list.items():
            ...
            # ── 데이터 + 지표 ──────────────────────────────────
            try:
                df = get_ohlcv_dataframe(mock_broker, symbol, days=120)
                if df.empty:
                    slog(f"[{name}] 데이터 없음, 스킵")
                    continue
                df = calculate_indicators(df)
                # 스토캐스틱 추가 (백테스트와 동일 파라미터)
            ...
            if symbol in positions:
                ...
                reason = check_exit_condition(df, entry_price, sl_pct, rsi_exit)
            ...
            if daily_count >= MAX_DAILY_TRADES:
                slog(f"오늘 최대 매매 횟수({MAX_DAILY_TRADES}회) 도달 — 신규 진입 중단")
                break

            # 상승장 필터
            if use_bull_filter and not is_bull_market_filter(df):
                slog(f"[{name}] 상승장 필터 미통과 (20일선 하락 추세) — 스킵")
                continue

            # 3중 진입 조건
            if not check_scalping_signal_daily(df, rsi_thr, stoch_thr):
                last = df.iloc[-1]
                slog(
                    f"[{name}] 진입 조건 미충족 "
                    f"(종가={float(last['close']):,.0f}, "
                    f"BB하단={float(last.get('bb_lower', 0)):,.0f}, "
                    f"RSI={float(last.get('rsi', 0)):.1f}, "
                    f"Stoch={float(last.get('stoch_k', 0)):.1f})"
                )
                continue
            ...
            if qty < 1:
                slog(f"[{name}] 예수금 부족 (가능금액={max_invest:,}원, 주가={cur_price:,}원) — 스킵")
                continue

            # 진입 신호 확정 → 15:20까지 대기 후 시장가 매수
            last_row = df.iloc[-1]
            rsi_val   = round(float(last_row.get("rsi",     0)), 1)
            stoch_val = round(float(last_row.get("stoch_k", 0)), 1)
            wait_secs = max(0, (SCALP_ORDER_MIN - now.minute) * 60 - now.second)
            slog(f"[{name}] 진입 신호! RSI={rsi_val} Stoch={stoch_val} — {wait_secs}초 후 매수")
            time.sleep(wait_secs)
            ...
            if resp:
                ...
                _append_trade_log({
                    ...
                    "qty": qty, "rsi": rsi_val, "stoch_k": stoch_val,
                })
                positions[symbol] = {
...
if start_btn:
    if _state["thread"] is None or not _state["thread"].is_alive():
        ...
        _state["thread"] = threading.Thread(
            ...
            args=(
                _state, _load_watchlist(),
                st.session_state.get("scalp_rsi_exit_val", rsi_thresh),
                st.session_state.get("scalp_stoch_val", stoch_thresh),
                scalp_sl_pct, scalp_rsi_exit, scalp_bull_filter,
            ),
            daemon=True,
        )
        _state["thread"].start()
        st.success("스캘핑 봇 시작 완료")
    else:
...
st.markdown(f"**봇 상태:** {status_color} {bot_status} &nbsp;|&nbsp; **오늘 매매:** {daily_count}/{MAX_DAILY_TRADES}회")
